...
# use RPI GPU
ffmpeg -r 30 -use_wallclock_as_timestamps 1 -thread_queue_size 512  -f v4l2 -i /dev/video0  -c:v h264_omx -maxrate 768k  -vf "format=yuv444p,drawbox=y=ih-h:color=black@0.9:width=40:height=12:t=max,drawtext=fontfile=OpenSans-Regular.ttf:textfile=/tmp/meta.txt:reload=1:fontsize=10:fontcolor=white:x=0:y=(h-th-2),format=yuv420p" -threads 4 -r 30 -g 60 -f flv rtmp://newstaging.cometa.io:12345/src/74DA388EAC61
