...
def pipeline(image):
    ...
    if lineRight.line_detected == False:
        centroid_starter_right = lineRight.find_starter_centroids(warped_img, x0=warped_img.shape[1]/2, peak_thresh=peak_thresh)
        lineRight.starter_centroid = centroid_starter_right['centroid']
        lineRight.line_detected = True
        
   
    log_lineRight = lineRight.run_sliding_window(warped_img, lineRight.starter_centroid, sliding_window_specs)    
